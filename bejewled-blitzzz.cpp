...
int menu()//This function will print the initial menu which shows help, play and quit options. 
{
	...
	while (1)//The loop will not terminate until you quit or enter the game.
	{
		Sleep(300);
...
int scorecount(int array[][8])//This function will count the score of player.
{
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			if (array[i][j] == 13) //empty block showing score
				score = score + 50;
		}
	}
	return score;
}
bool cubejewelmaker(int x[][8])//This function will create the special cube jewel.(destroyer)
{
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
...
bool firejewelmaker(int array[][8])//This function will create fire jewel of respected jewel(if we get four similar jewels in X or Y axis). (Flame gem) 
{
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
		{    
	...
	for (int i = 0; i < 8 ; i++)
		for (int j = 0; j < 8; j++)
		{
			...
			if (firejewelmaker(x) == true)//creating fire jewel.
				return true;
			if (firejewelcrusher3(x[i][j], x[i][j + 1], x[i][j + 2]) == true && x[i][j] != 13) //3 by 3 crusher
			{//X axis.
...
{
	srand(time(0)); //gives time as a seed 
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			if (x[i][j] == 13)
			{
				...
				while (x[i][j] == x[i][j - 1] || x[i][j] == x[i][j + 1] || x[i][j] == x[i - 1][j] || x[i][j] == x[i + 1][j])
					x[i][j] = rand() % 6; //random number generate
			}
}
...
void gameplay(int array[][8])//This function will control all the tasks taking place in the loop.
{
	...
	{  //Timer.
		...
		c = GetAsyncKeyState(38); //cursor movement
		if (c == 1 && y > 30)
		{
			y = y - 50; y1 = y1 - 50; i--;
		}
		c = GetAsyncKeyState(37);
		if (c == 1 && x > 550) //c tells key is pressed, 
		{
			x = x - 50; x1 = x1 - 50; j--;
		}
